...
porque el problema es distinto al que resuelve GraphImpactAnalyzer:
  - graph_impact.py responde "¿qué se cae si corto este cable/equipo?"
    (alcanzabilidad + compuertas AND/OR sobre un espejo en Python puro,
    pensado para miles de simulaciones puntuales muy rápidas).
  - Este módulo responde "¿qué señal hay en cada conector, dado lo que
    ...
    contenido, una sola pasada sobre todo el grafo): la escala (miles
    de conectores) resuelve cómodo en Python puro con una relajación
    iterativa tipo Bellman-Ford.
