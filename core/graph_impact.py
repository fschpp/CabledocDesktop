...
=================================================================================
Modela el grafo dirigido de señal (Equipo-OUT --[CABLE]--> Equipo-IN) y calcula
alcanzabilidad con un espejo en Python puro (self._adj) armado en
construir_grafo(): sin dependencias externas más allá de la librería estándar.
...
# Por defecto, SQLite escribe las tablas temporales que no entran en
# memoria (por ejemplo en consultas pesadas sobre bases grandes) en la
# carpeta temporal del sistema operativo, que en Linux suele ser /tmp — a
# veces un tmpfs con tamaño limitado por RAM y NO por el disco real. Si esa
# carpeta se llena, SQLite falla con "database or disk is full" aunque el
# disco donde vive la app tenga espacio de sobra. Acá la redirigimos a una
# carpeta local, en el mismo disco que db.db, antes de que se abra
# ninguna conexión SQLite de este módulo.
...
class GraphImpactAnalyzer:
    ...
    Internamente usa un espejo del grafo en Python puro (self._adj), sin
    dependencias externas. La simulación es no destructiva: se elimina el
    edge, se corre BFS parcial
    sobre las fuentes afectadas, y se restaura el edge antes de retornar.
    """

    def __init__(self, db_path: str):
        self._db_path = db_path
        self._grafo_construido: bool = False
        ...
        # aplicar las reglas lógicas (AND/OR) durante el recorrido, algo
        # que una simple alcanzabilidad de grafo no resuelve por sí sola.
    ...
    def construir_grafo(self) -> None:
        """
        Lee la BD de CableDoc, arma el espejo del grafo en self._adj
        y pre-calcula el BFS de estado normal desde cada fuente de señal.

        Tiempo típico: ~75 ms (10 ms lectura + 65 ms BFS inicial).
        """
        ...
        # señal, agregadas como propiedades ADICIONALES de la arista CABLE
        # ya existente (quedan disponibles en self._adj/edges) — no
    ...
    def _alcanzables_desde(self, semillas: set, aristas_excluidas: frozenset = frozenset(),
        ...
        (así se simula un corte sin reconstruir el grafo).
    ...
    def _leer_bd(self) -> tuple[list[dict], list[dict], set[str], set[str],
        ...
        # de a un id_equipo. Ese nodo de paso participa de self._adj igual
        # que cualquier otro nodo — el bucle de armado de aristas de más abajo
    ...
    def simular_desconexion(self, cable_id: str) -> ResultadoImpacto:
        ...
        Estrategia (no destructiva, no reconstruye el grafo):
    ...
    def simular_falla_equipo(self, equipo_id: str) -> ResultadoImpactoEquipo:
        ...
        self._adj): se recalcula la alcanzabilidad global excluyendo
    ...
    def simular_escenario(self, cables_cortados=(), equipos_fallados=(),
        ...
        Estrategia no-destructiva (no reconstruye el grafo ni toca self._adj), igual
