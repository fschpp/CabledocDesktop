...
- **Motor de grafos**: espejo en Python puro (`self._adj`), sin dependencias externas, usado por `graph_impact.py` (clase `GraphImpactAnalyzer`). La vía histórica por GraphQLite/Neo4j (`cypher_console.py`, consola de consultas Cypher) fue retirada del desktop.
...
| `impacto_ui.py`, `diagrama_personalizado.py`, `acerca_de.py`, `bitacora_ui.py`, `escenario_ui.py`/`escenario_engine.py`, `diagnostico_ui.py`/`diagnostico_falla.py`, `i18n.py` | Módulos UI/soporte especializados |
...
- **Grafos**: espejo en Python puro (sin dependencias externas). La vía histórica por GraphQLite/Neo4j (`cypher_console.py`) fue retirada del desktop.
