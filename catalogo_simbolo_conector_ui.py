...
tipo_conector, reutilizable entre todos los equipos. No depende de ninguna
...
alta/edición, con combo buscable de tipo_conector y validación de XML antes
...
class _DialogoSimboloConector(Gtk.Dialog):
    def __init__(self, parent=None, fila=None):
        """`fila` = (id, id_tipo_conector, svg_fragmento, viewbox,
        ...
        self.valor = None  # (id_tipo_conector, frag, viewbox, tam_rel, color) al aceptar

        g = _grid()
        fila_grid = 0

        g.attach(Gtk.Label(label=_("Tipo de conector:"), xalign=0), 0, fila_grid, 1, 1)
        self.c_tipo = _searchable_combo(g, fila_grid, Modelo.devolver_tipos_conectores())
        if fila:
    ...
    def _on_response(self, dialogo, response_id):
        ...
        if not id_tipo:
            mostrar_error(self, _("Elegí un tipo de conector."))
...
class CatalogoSimbolosConectorListado(VentanaListado):
    ...
    def __init__(self, parent=None, modo_seleccion=False):
        super().__init__(
            ...
            [_("ID"), _("Tipo de conector"), _("Tamaño relativo"), _("Fragmento (caract.)")],
    ...
    def nuevo(self):
        ...
        if dlg.run() == Gtk.ResponseType.OK and dlg.valor:
            ...
            existente = Modelo.devolver_simbolo_de_tipo_conector(id_tipo)
            if existente:
                mostrar_error(
                    self,
                    _("Ya existe un símbolo para ese tipo de conector — "
                      "abrilo para editarlo en vez de crear uno nuevo."))
            else:
