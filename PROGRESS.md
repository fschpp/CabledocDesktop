...
renderizado (reemplazó a Graphviz), espejo en Python puro como motor de
análisis de grafos/impacto (`graph_impact.py`). La vía histórica por
GraphQLite/Neo4j (`cypher_console.py`) fue retirada del desktop.
...
| `graph_impact.py` | Análisis de impacto/propagación de fallas (`GraphImpactAnalyzer`, espejo en Python puro) |
...
### Current Focus
- [x] Re-clonado `main` (había avanzado de `a21c60b` a `0e1db60` con las Fases 6-9 de `plan_desarrollo_ubicacion_fisica_planos.md`, mergeadas por otra sesión/otra persona mientras se trabajaba esto) — el trabajo de Fase 0/1 se había hecho primero sobre `a21c60b`, se re-aplicó completo contra `0e1db60`.
- [x] `modelo.py`: tabla `catalogo_simbolo_conector` (símbolo por tipo de conector, reutilizable) + CRUD completo; `asegurar_columnas_dimensiones()` (ancho_mm/alto_mm/profundidad_mm en equipo/equipo_catalogo/frame/frame_catalogo, mm_por_pixel en imagen); `resolver_mm_por_pixel()` (cascada de 3 fuentes: medición manual > ancho_mm de la entidad > None) y `calcular_radio_simbolo_px()` (fórmula del plan §3.2). Getters/setters de dimensiones con whitelist de tabla (protegido contra SQL injection, probado explícitamente).
- [x] `pantallas_comunes.py`: helpers compartidos `_crear_handle_simbolo`/`_dibujar_simbolo_conector` (Rsvg real, fallback silencioso ante SVG corrupto).
- [x] `catalogo_simbolo_conector_ui.py` (archivo nuevo): pantalla de administración con vista previa en vivo del símbolo y validación de XML antes de guardar. Enganchada al menú Catálogos en `cabledoc.py`.
- [x] `imagen_conectores_ui.py` y `editor_masivo_conectores_ui.py`: Fase 1 completa — SELECT extendido con `id_tipo_conector`/`id_imagen` (sin romper índices existentes), cálculo de escala precomputado una sola vez por carga (no por redibujo), render condicional con fallback automático al marcador/círculo genérico si falta símbolo o calibración.
- [x] `schema_db.sql` actualizado para instalaciones frescas (las 5 tablas ya traen las columnas nuevas sin depender de la migración idempotente).
- [x] Validado en sandbox: `ast.parse` + `pyflakes` (comparado contra `git show HEAD:archivo.py`, cero advertencias nuevas en los 3 archivos preexistentes tocados) en los 7 archivos tocados/creados; smoke tests funcionales (SQL real contra una base construida desde `schema_db.sql`, sin levantar ventanas GTK) de: alta/listado/lookup de símbolos, whitelist de dimensiones con el caso adversarial `'DROP TABLE equipo;--'`, cascada de calibración en sus 3 fuentes, cálculo de radio (XLR vs BNC), `_crear_handle_simbolo`/`_dibujar_simbolo_conector` con Rsvg/Cairo reales bajo Xvfb (fragmento válido y fragmento roto), `_cargar_datos` de las dos subclases del editor masivo trayendo `id_tipo_conector`.
- [ ] **Pendiente — smoke test visual con GTK real (a cargo de Papi, indicado explícitamente por él):** instanciar las ventanas completas, abrir el diálogo de símbolo con `.run()`, verificar el render sobre una imagen SVG real dentro de `ImagenConectoresYCables`/`EditorMasivoConectoresBase` contra `database/db.db` real. Xvfb se cayó de forma intermitente entre llamadas de herramienta en este sandbox — no se insistió más allá de la validación funcional sin ventana ya descripta arriba.
- [ ] **Pendiente — no implementado en esta entrega:** herramienta "Medir en la imagen" (§3.4 del plan, fuente 1 de calibración manual) — sólo se implementó el guardado/lectura de `imagen.mm_por_pixel` (`obtener_mm_por_pixel_imagen`/`actualizar_mm_por_pixel_imagen`), falta la UI de marcar dos puntos + diálogo de mm reales.
- [ ] **Pendiente — no implementado en esta entrega:** copiado automático de `ancho_mm`/`alto_mm`/`profundidad_mm` al instanciar un equipo desde un molde de catálogo (`Modelo.instanciar_desde_catalogo`) ni al crear un catálogo desde un equipo (`crear_catalogo_desde_equipo`) — se decidió no tocar esas firmas en esta entrega por el riesgo/alcance (ver Latest Blockers/Discoveries). Hoy las dimensiones de equipo/molde son independientes entre sí una vez creadas.
- [ ] **Pendiente — UI de edición de dimensiones:** no se agregó ningún campo/diálogo en las pantallas de equipo/equipo_catalogo/frame/frame_catalogo para cargar ancho_mm/alto_mm/profundidad_mm a mano (ni el atajo "usar 19″ estándar") — sólo existen los métodos de `Modelo`; falta la parte visible al usuario.

### Latest Blockers/Discoveries
- El mismo blocker recurrente de este sandbox ya documentado en sesiones anteriores: `Xvfb` lanzado en background no sobrevive entre invocaciones separadas de herramienta.
- Decisión de diseño explícita: **no se modificó la firma de `alta_equipo`/`modificacion_equipo`/`alta_catalogo`/`modificacion_catalogo`/`instanciar_desde_catalogo`** para agregar las dimensiones ahí — son funciones con muchos call sites en archivos grandes (`equipos_ui.py`, `equipos_alta_rapida_ui.py`, `catalogo_equipos_alta_rapida_ui.py`, etc.) y sin poder ejercitar un smoke test visual completo en este entorno, el riesgo de tocarlas a ciegas se consideró mayor que el beneficio de tener el copiado automático ya en esta entrega. Quedó como getters/setters independientes y aditivos — sin romper nada existente, pero con el follow-up de la copia automática pendiente (ver arriba).
- Mientras se armaba esta entrega, `main` avanzó con las Fases 6-9 de `plan_desarrollo_ubicacion_fisica_planos.md` (otro plan, en curso en paralelo) — el trabajo de Fase 0/1 de paneles vectoriales se había hecho primero contra un `main` más viejo (`a21c60b`) y hubo que re-aplicarlo a mano contra el `main` real (`0e1db60`): 4 de 6 archivos entraron con `git apply` sin tocar nada (`modelo.py` con un offset de 1 línea, `pantallas_comunes.py`/`editor_masivo_conectores_ui.py`/`imagen_conectores_ui.py`/`schema_db.sql` limpios), y sólo `cabledoc.py`/`changelog.txt` necesitaron reaplicarse a mano por los conflictos esperables (`APP_VERSION` y menú/migraciones tocados por la Fase 9 del otro plan). Ninguno de los archivos de paneles vectoriales (`catalogo_simbolo_conector_ui.py`, la lógica nueva de `modelo.py`) se solapa con lo que tocó el otro plan (planos/salas/muebles), así que no hubo conflicto semántico, sólo de líneas de contexto.
